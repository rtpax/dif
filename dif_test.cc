...
TEST_CASE("finalize_dif - preserve", "no changes") {
    ...
    dif d = finalize_dif(dh,"abcd","abcd");

    CHECK(d.ds.size() == 1);
    auto it = d.ds.begin();

    CHECK(it->type == dif_segment::preserved);
    CHECK(it->s == "abcd");
}

TEST_CASE("finalize_dif - delete", "deletioin, no insetion") {
    dif_hist dh{{dif_hist::del},{},0};
    dif d = finalize_dif(dh,"abcd","abd");
    ...
    CHECK(it->type == dif_segment::deletion);
    CHECK(it->s == "c");
    ++it;
...
TEST_CASE("finalize_dif - insert", "insertion, no deletion") {
    ...
    dif d = finalize_dif(dh,"abcd","aXbcd");
    ...
    CHECK(it->type == dif_segment::insertion);
    CHECK(it->s == "X");
    ++it;
...
TEST_CASE("finalize_dif - modify", "insertion and deletion") {
    ...
    dif d = finalize_dif(dh,"abcd","aXbd");
...
TEST_CASE("finalize_dif - replace", "abcdefg -> abcdXfg.") {
    ...
    dif d = finalize_dif(dh,"abcdefg","abcdXfg");
...
TEST_CASE("paths_at_end", "abcd -> abd: only 4,3 works") {
    ...
    auto end_paths = paths_at_end(paths, "abcd", "abd");
